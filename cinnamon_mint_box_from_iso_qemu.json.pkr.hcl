...
source "qemu" "autogenerated_1" {
  boot_command     = ["<esc><wait><tab><wait><leftCtrlOn>u<leftCtrlOff>", "/casper/vmlinuz initrd=/casper/initrd.lz", " preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/linux_mint_preseed.cfg<wait>", " automatic-ubiquity debug-ubiquity boot=casper nosplash noprompt", " -- ", "<enter><wait15m>", "<leftCtrlOn><leftAltOn>t<leftAltOff><leftCtrlOff><wait>", "echo 'pulsar' | sudo -S apt update<enter><wait60>", "echo 'pulsar' | sudo -S apt install -y openssh-server build-essential<enter><wait60>", "echo 'pulsar' | sudo -S bash -c 'echo \"pulsar ALL=(ALL) NOPASSWD: ALL\" > /etc/sudoers'<enter><wait10>", "echo 'pulsar' | sudo -S reboot now<enter><wait120>"]
  disk_size        = "40960"
  http_directory   = "http"
  ...
  ssh_port         = 22
  ssh_username     = "pulsar"
  vm_name          = "linuxmint-21.3-cinnamon_qemu"
}
...
build {
  ...
  provisioner "file" {
    source = "bashrc"
    destination = "/home/pulsar/.bashrc"
  }
  provisioner "shell" {
    environment_vars = ["TEMPO2_DIR=/home/pulsar/software/tempo2","TEMPO2=/home/pulsar/software//tempo2/install"]
    inline = [
      ...
      "apt-get update",
      "apt-get --no-install-recommends -y install build-essential autoconf autotools-dev automake autogen libtool pkg-config cmake csh g++ gcc gfortran wget git expect libcfitsio-dev hwloc perl pcre2-utils libpcre2-dev pgplot5 python3.10 python3-dev python3-testresources python3-pip python3-setuptools python3-tk libfftw3-3 libfftw3-bin libfftw3-dev libfftw3-single3 libx11-dev libpcre3 libpcre3-dev libpng-dev libpnglite-dev libhdf5-dev libhdf5-serial-dev libxml2 libxml2-dev libltdl-dev libffi-dev libssl-dev libxft-dev libfreetype6-dev libblas-dev liblapack-dev libatlas-base-dev libsuitesparse-dev gsl-bin libgsl-dev bc",
      ...
      "pip install -U cython scikit-image  numpy==1.23.0",
      "pip install -U pandas matplotlib astropy scipy",
      ...
      "./configure --prefix=$TEMPO2 --with-x --x-libraries=/usr/lib/x86_64-linux-gnu --with-fftw3-dir=/usr/ --with-calceph=$CALCEPH/install/lib --enable-shared --enable-static --with-pic CPPFLAGS=\"-I\"$CALCEPH\"/install/include -L\"$CALCEPH\"/install/lib/ -I\"$PGPLOT_DIR\"/include/ -L\"$PGPLOT_DIR\"/lib/\" CXXFLAGS=\"-I\"$CALCEPH\"/install/include -L\"$CALCEPH\"/install/lib/ -I\"$PGPLOT_DIR\"/include/ -L\"$PGPLOT_DIR\"/lib/\"",
      "make -j $(nproc)",
      ...
      "cd $PSRCHIVE_DIR",
      "./bootstrap",
      "./configure --prefix=$PSRCHIVE --with-x --x-libraries=/usr/lib/x86_64-linux-gnu --with-psrxml-dir=$PSRXML/install --enable-shared --enable-static F77=gfortran PYTHON=$(which python3) CPPFLAGS=\"-I\"$CALCEPH\"/install/include -L\"$CALCEPH\"/install/lib/ -I\"$PGPLOT_DIR\"/include/ -L\"$PGPLOT_DIR\"/lib/\" CXXFLAGS=\"-I\"$CALCEPH\"/install/include -L\"$CALCEPH\"/install/lib/ -I\"$PGPLOT_DIR\"/include/ -L\"$PGPLOT_DIR\"/lib/\" LDFLAGS=\"-L\"$PSRXML\"/install/lib -L\"$CALCEPH\"/install/lib/  -L\"$PGPLOT_DIR\"/lib/ \" LIBS=\"-lpsrxml -lxml2\"",
      "make -j $(nproc)",
      "make install",
      "make clean",
      "cd $PSRHOME",
